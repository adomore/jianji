...
struct AddSheetView: View {
    ...
    /// Blinking insertion caret. Driven by the wall clock via TimelineView so it stays
    /// visible no matter which position it's re-created at (a plain state-toggle blink goes
    /// invisible on freshly-inserted instances).
    ...
    private func save(amount: Decimal, category: Category?, date: Date, note: String,
        ...
        context.insert(tx)
        try? context.save()
        BudgetEvaluator.run(context)     // 记一笔后检查是否触及预算阈值
        WidgetBridge.update(context)     // 同步桌面小组件
        Haptics.success()
        dismiss()
    }
