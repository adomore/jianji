...
struct ChartsView: View {
    ...
    /// Last 6 months (ending at the displayed month) of expense + income for the active book,
    /// flattened into per-series points for a grouped bar chart.
    private var trendPoints: [TrendPoint] {
        let ledgerAll = all.filter { $0.ledger?.id == activeLedger?.id && (privacy.revealed || !$0.isPrivate) }
        var points: [TrendPoint] = []
        for back in (0..<6).reversed() {
            let m = cal.date(byAdding: .month, value: -back, to: month) ?? month
            let mtx = ledgerAll.filter { cal.isDate($0.date, equalTo: m, toGranularity: .month) }
            let e = mtx.filter { $0.isExpense }.reduce(Decimal(0)) { $0 + $1.amount }
            let i = mtx.filter { !$0.isExpense }.reduce(Decimal(0)) { $0 + $1.amount }
            let label = "\(cal.component(.month, from: m))月"
            points.append(TrendPoint(label: label, kind: "支出", amount: Double(truncating: e as NSDecimalNumber)))
            points.append(TrendPoint(label: label, kind: "收入", amount: Double(truncating: i as NSDecimalNumber)))
        }
        return points
    }
    private var trendHasData: Bool { trendPoints.contains { $0.amount > 0 } }
    ...
    private var trendCard: some View {
        VStack(alignment: .leading, spacing: 12) {
            Text("近半年收支趋势").font(.system(size: 16, weight: .semibold)).foregroundStyle(t.text)
            if !trendHasData {
                emptyHint("近 6 个月还没有记录")
            } else {
                Chart(trendPoints) { p in
                    ...
                    .position(by: .value("类型", p.kind))
                }
                .frame(height: 180)
                .chartForegroundStyleScale(["支出": t.red, "收入": t.green])
...
/// One (month, series) bar for the 6-month expense/income trend chart.
struct TrendPoint: Identifiable {
    let id = UUID()
    let label: String   // "7月"
    let kind: String    // "支出" / "收入"
    let amount: Double
}
