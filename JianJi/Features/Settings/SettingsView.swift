            ...
            List {
                Section {
                    ...
                    NavigationLink { RecurringListView() } label: {
                        row("arrow.triangle.2.circlepath", Color(hex: "FF9500"), "周期账单")
                    }
                } header: { Text("记账") }
                ...
                Section {
                    ...
                    NavigationLink { ICloudSyncView() } label: {
                        HStack {
                            row("icloud.fill", Color(hex: "007AFF"), "iCloud 同步")
                            Spacer()
                            Text(SyncStatus.iCloudActive ? "已开启" : "未启用")
                                .font(.system(size: 13))
                                .foregroundStyle(SyncStatus.iCloudActive ? t.green : t.ter)
                        }
                    }
                } header: { Text("更多") } footer: {
                    Text(SyncStatus.iCloudActive
                         ? "账单已通过 iCloud 在你的设备间自动同步。"
                         : "同一 Apple ID 的设备间自动同步账单（需开启 iCloud 能力）。")
                }
        ...
        List {
            Section {
                VStack(spacing: 8) {
                    ...
                    Text("版本 1.1").font(.system(size: 13)).foregroundStyle(t.sec)
                }
            ...
            Section {
                Text("未开启 iCloud 时数据仅存本机，删除 App 即清空；开启 iCloud 后账单会同步到你的 iCloud。")
