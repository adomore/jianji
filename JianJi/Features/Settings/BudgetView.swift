...
struct BudgetView: View {
    ...
    @AppStorage("budgetReminderEnabled") private var reminderOn = false
    ...
    var body: some View {
        List {
            ...
            if budget > 0 {
                ...
                Section {
                    Toggle("预算提醒", isOn: $reminderOn)
                        .onChange(of: reminderOn) { _, on in
                            if on {
                                NotificationManager.shared.requestAuthorization { granted in
                                    if !granted { reminderOn = false }   // 未授权则回退
                                    else { BudgetEvaluator.run(context) }
                                }
                            }
                        }
                } footer: {
                    Text("用量达 80% 与超支时各推送一次本地通知（不涉及隐私账单）。")
                }
