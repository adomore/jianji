...
- 账本页可**切换 / 新建 / 改名换图标颜色 / 删除 / 长按拖动排序**（`onMove` 写回 `sortOrder`，无需 Edit 按钮）；删除某账本时其账单移入「默认账本」（不删账单），默认账本不可删——沿用分类「其他」的规则。
- **每个账本单独报表** `LedgerReportView`：点账本行进入，按该账本作用域显示月度卡片 + 明细/图表分段（图表为分类排行条形）；顶部「设为当前」可切换活动账本。
- **记账页顶部账本切换**：`ledgerMenu` 下拉直接切当前账本，新账单即记入该账本，无需跳去账本页。
